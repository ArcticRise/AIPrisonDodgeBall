...
public class MinionThrowTester : MonoBehaviour
{
    ...
    public static bool PredictThrow(
        ...
        out float altT)
    {
        var soln = LoC(projectilePos, maxProjectileSpeed * 0.9f, projectileGravity, targetInitPos, targetConstVel, targetForwardDir, maxAllowedErrorDist, out Vector3 projectileDir2, out float projectileSpeed2, out float interceptT2, out float altT2);
        altT = altT2;
        projectileDir = Vector3.zero;
        interceptT = -1f;
        projectileSpeed = -1f;
